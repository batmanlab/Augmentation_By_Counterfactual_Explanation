...
import score_calculation as lib


parser = argparse.ArgumentParser(description='Evaluates a CIFAR OOD Detector',
                                 formatter_class=argparse.ArgumentDefaultsHelpFormatter)
# User params
parser.add_argument('--test_name', default='afhq', type=str)
parser.add_argument('--TS', default='yes', type=str)
parser.add_argument('--score', default='Odin', type=str, help='score options: M|Odin|energy')
parser.add_argument('--num_classes', default=2, type=int)
parser.add_argument('--in_channels', default=3, type=int)
parser.add_argument('--chkpt_path', default='/jet/home/nmurali/asc170022p/singla/CounterfactualExplainer/MIMICCX-Chest-Explainer/Classifier/torchxrayvision_/Experiment_AFHQ/AFHQ_256/DenseNet_Cat_Dog_seed_1234/AFHQ-densenet169-AFHQ_256-best.pt', type=str)
...
net.load_state_dict(torch.load(args.chkpt_path).state_dict())
net.eval()
net.cuda()
...
# get temperature
if args.TS=='yes':
    new_model = ModelWithTemperature(net, 0.0)
    new_model.set_temperature(test_loader)
    args.T = new_model.temperature.item()
    print('new temperature is: %f' %(args.T))
...
def get_and_print_results(ood_loader, num_to_avg=args.num_to_avg):
    ...
    for _ in range(num_to_avg):
        if args.score == 'Odin':
            ...
            out_score = lib.get_Mahalanobis_score(net, ood_loader, args.num_classes, args.in_channels, sample_mean, precision, count-1, args.noise, num_batches)
        elif args.score=='energy':
            out_score = get_ood_scores(ood_loader)
        else:
            raise('Wrong args.score')
...
if args.score == 'Odin':
    ...
    in_score = lib.get_ood_scores_odin(test_loader, net, args.test_bs, ood_num_examples, args.T, args.noise, in_dist=True)
elif args.score == 'M':
    ...
    try:
        ...
        try:
            temp_x = torch.rand(2,1,32,32)
            temp_x = Variable(temp_x)
            temp_x = temp_x.cuda()
            temp_list = net.feature_list(temp_x)[1]
        except:
            raise('Error in obtaining net.feature_list!')
    ...
    sample_mean, precision = lib.sample_estimator(net, args.num_classes, feature_list, test_loader) 
    in_score = lib.get_Mahalanobis_score(net, test_loader, args.num_classes, args.in_channels, sample_mean, precision, count-1, args.noise, num_batches, in_dist=True)
...
# print('Test Accuracy {:.2f}'.format(100 * num_right / (num_wrong + num_right)))
