...
def get_ood_scores(loader, in_dist=False):
    ...
    with torch.no_grad():
        for batch_idx, samples in enumerate(loader):
            try:
                data, target = samples['img'], samples['lab']
            except:
                try:
...
if args.score == 'Odin':
    ...
    if 'cifar10_' in args.method_name:
        train_data = dset.CIFAR10('../data/cifarpy', train=True, transform=test_transform, download=True)
    else:
        train_data = dset.CIFAR100('../data/cifarpy', train=True, transform=test_transform, download=True)
