...
def show_performance(pos, neg, method_name='Ours', recall_level=recall_level_default):
    ...
    print('\t\t\t' + method_name)
    print('TNR{:d}:\t\t\t{:.2f}'.format(int(100 * recall_level), 100 * (1-fpr)))
...
def print_measures(auroc, aupr, fpr, method_name='Ours', recall_level=recall_level_default):
    print('\t\t\t\t' + method_name)
    print('  TNR{:d} AUROC AUPR'.format(int(100*recall_level)))
    print('& {:.2f} & {:.2f} & {:.2f}'.format(100*(1-fpr), auroc, 100*aupr))
...
def print_measures_with_std(aurocs, auprs, fprs, method_name='Ours', recall_level=recall_level_default):
    print('\t\t\t\t' + method_name)
    print('  TNR{:d} AUROC AUPR'.format(int(100*recall_level)))
    print('& {:.2f} & {:.2f} & {:.2f}'.format(100*(1-np.mean(fprs)), np.mean(aurocs), 100*np.mean(auprs)))
    print('& {:.2f} & {:.2f} & {:.2f}'.format(100*np.std(fprs), np.std(aurocs), 100*np.std(auprs)))
